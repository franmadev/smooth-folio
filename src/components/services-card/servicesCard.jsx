    ...
    <>
      <article className="bg-grey py-3 my-6 rounded-md shadow-lg flex ">
        <div className="w-[100%] relative">
          <h1 className="px-4 font-semibold">{serviceData.title}</h1>
          <p className="px-4 text-softwhite w-[100%] ">{serviceData.description}</p>
          <div className="place-content-between flex px-4">
            <div></div>
            <div>
              <button
               // onClick={() => navigate(`/servicios/${serviceData.titleId}`, {state:serviceData})}
               onClick={() => navigate(`/servicios/${serviceData.titleId}`)}
